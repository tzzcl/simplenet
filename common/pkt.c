...
int son_sendpkt(int nextNodeID, sip_pkt_t* pkt, int son_conn)
{
	...
	if (send(son_conn,&nextNodeID,sizeof(int),0)<=0) goto bad;
	if (send(son_conn,pkt,PKT_HEADER_LENGTH+nowlength,0)<=0) goto bad;
	if (send(son_conn,"!#",2,0)<=0) goto bad;
...
{
	int state=0;
	sendpkt_arg_t temp__;
	sendpkt_arg_t *temp=&temp__;
...
int forwardpktToSIP(sip_pkt_t* pkt, int sip_conn)
{
	...
	int nowlength=pkt->header.length;
	if (send(sip_conn,pkt,PKT_HEADER_LENGTH+nowlength,0)<=0) goto bad;
	if (send(sip_conn,"!#",2,0)<=0) goto bad;
...
sip_pkt_t* build_failpkt(int nodeID) {
	...
	hdr->src_nodeID=BROADCAST_NODEID;
